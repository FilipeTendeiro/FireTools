...
def get_subprocess_env():
    """
    Get environment for subprocess with proper PYTHONPATH.
    Ensures subprocess can import required modules.
    """
    env = os.environ.copy()
...
def url_to_path(url_path: str) -> str:
    ...
    # Check if it's a Linux absolute path (any path starting with /)
    if url_path.startswith('/'):
        # Extract the relative part and reconstruct using current config paths
        # This handles cases where paths from different server instances are passed
        if 'frames/' in url_path:
            filename = url_path.split('frames/')[-1]
            return str(FRAMES_DIR / filename)
        elif 'segmentation/' in url_path:
            rel_part = url_path.split('segmentation/')[-1]
            return str(SEGMENTATION_DIR / rel_part)
            ...
            # If no known directory in path, try to extract just the filename
            # and check if it looks like a frame file
            filename = os.path.basename(url_path)
            if '_frame_' in filename and filename.endswith('.jpg'):
                return str(FRAMES_DIR / filename)
...
async def process_file(task_id: str, file_path: str, original_filename: str):
    ...
    try:
        ...
        process = await asyncio.create_subprocess_exec(
            "python3", str(Path(__file__).parent / "run_inference.py"), file_path,
        ...
        print(f"[DEBUG] Task {task_id}: Command executed: python3 {Path(__file__).parent / 'run_inference.py'} {file_path}")
        ...
        gallery_entry = {
            "local_result_path": main_result_path,
            "local_original_path": original_display_path,
            "output_folder": str(item_folder),
...
async def process_segmentation(task_id: str, input_path: str, original_filename: str):
    ...
    try:
        ...
        if input_type == "video":
            ...
            segmentation_cmd = (
                f"python3 {SEGMENTATION_CONFIG['script_path']} "
                f"--video {input_path} "
        ...
        if input_type == "video":
            cmd_args = [
                "python3", str(SEGMENTATION_CONFIG['script_path']),
                "--video", input_path,
        ...
        gallery_entry = {
            "local_original_path": input_path,
            "local_mask_path": str(mask_file) if mask_file else None,
            "local_overlay_path": str(overlay_file) if overlay_file else None,
            "local_detection_path": str(detection_file) if detection_file else None,
            "output_dir": str(output_dir),
...
async def get_gallery(media_type: Optional[str] = None, limit: Optional[int] = None):
    ...
    # Convert old absolute paths to relative URLs if needed (backward compatibility)
    for item in gallery:
        if 'local_result_path' in item and item['local_result_path'].startswith('/'):
            item['local_result_path'] = path_to_url(item['local_result_path'])
        if 'local_original_path' in item and item['local_original_path'].startswith('/'):
...
async def delete_gallery_item(item: GalleryItemDelete):
    ...
    # Normalise stored paths to relative URLs (same as GET /gallery/ does) before comparing,
    # because the frontend always receives and sends back relative URLs.
    item_index = -1
    for i, gallery_item in enumerate(gallery):
        stored_path = gallery_item["local_result_path"]
        if stored_path.startswith('/') and not stored_path.startswith('/outputs') and not stored_path.startswith('/uploads'):
            stored_path = path_to_url(stored_path)
...
async def start_segmentation(
    ...
    # Always convert/normalize the path through url_to_path
    # This ensures paths from different server instances are corrected
    original_path = input_path
    input_path = url_to_path(input_path)
    if input_path != original_path:
        print(f"[DEBUG] Converted path from '{original_path}' to '{input_path}'")
...
async def get_segmentation_gallery(limit: Optional[int] = None):
    ...
    # Convert old absolute paths to relative URLs if needed (backward compatibility)
    for item in gallery:
        if 'local_original_path' in item and item['local_original_path'] and item['local_original_path'].startswith('/'):
            item['local_original_path'] = path_to_url(item['local_original_path'])
        if 'local_mask_path' in item and item['local_mask_path'] and item['local_mask_path'].startswith('/'):
            item['local_mask_path'] = path_to_url(item['local_mask_path'])
        if 'local_overlay_path' in item and item['local_overlay_path'] and item['local_overlay_path'].startswith('/'):
            item['local_overlay_path'] = path_to_url(item['local_overlay_path'])
        if 'local_detection_path' in item and item['local_detection_path'] and item['local_detection_path'].startswith('/'):
            item['local_detection_path'] = path_to_url(item['local_detection_path'])
        if 'output_dir' in item and item['output_dir'].startswith('/'):
